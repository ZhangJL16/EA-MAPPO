...
from .config import NavigationConfig, apply_configuration_overrides
from .dynamics import integrate_double_integrator
from .telemetry_cost import TelemetryCostModel, TelemetryCostConfig
from .lidar import HorizontalLidarModel
from .scenario import NavigationScenario, ScenarioDefinition
...
class NavigationEnv(gym.Env[np.ndarray, np.ndarray]):
    """Checkpoint-compatible phase-1 goal-conditioned navigation environment.

    The large resource budget and per-step telemetry cost are retained only
    because they are part of the saved 77-dimensional observation and reward
    contract. They impose no finite-resource constraint or safety authority.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.scenario: ScenarioDefinition = NavigationScenario(scenario_name).definition
        base_config = NavigationConfig(world_size=self.scenario.world_size)
        ...
        self.energy_model = TelemetryCostModel(TelemetryCostConfig())
...
def make_navigation_env(
    ...
    **kwargs: Any,
    ...
    return NavigationEnv(scenario_name, **kwargs)
