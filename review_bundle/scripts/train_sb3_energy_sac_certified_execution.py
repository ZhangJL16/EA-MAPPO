...
class CertifiedExecutionMetricsCallback(EnergyMetricsCallback):
    def __init__(self, args, output_dir: Path) -> None:
        super().__init__(args, output_dir)
        self.kappa_takeovers = 0
        self.kappa_steps = 0
        self.charger_hold_steps = 0
        self.verified_departure_steps = 0
        self.policy_steps = 0

    def _on_step(self) -> bool:
        info = self.locals["infos"][0]
        self.kappa_takeovers += int(info["kappa_takeover_started"])
        authority = str(info["execution_authority"])
        self.kappa_steps += int(authority == "FROZEN_KAPPA")
        self.charger_hold_steps += int(authority == "CERTIFIED_CHARGER_HOLD")
        self.verified_departure_steps += int(authority == "POLICY_CERTIFIED_DEPARTURE_SUPPORT")
        self.policy_steps += int(authority == "POLICY_CERTIFIED_RUN_SUPPORT")
        return super()._on_step()

    def snapshot(self) -> dict[str, object]:
        snapshot = super().snapshot()
        snapshot["certified_execution_metrics"] = {
            "kappa_takeover_count": self.kappa_takeovers,
            "kappa_execution_steps": self.kappa_steps,
            "charger_hold_steps": self.charger_hold_steps,
            "verified_departure_steps": self.verified_departure_steps,
            "policy_certified_action_steps": self.policy_steps,
            "online_teacher_override_count": 0,
        }
        return snapshot
...
def run(args: argparse.Namespace) -> None:
    ...
    if sb3_version != "2.8.0":
        raise RuntimeError(f"certified execution requires stable-baselines3==2.8.0, found {sb3_version}")
    if args.flight_energy_multiplier != 2.0:
        raise ValueError("formal certified execution protocol requires flight_energy_multiplier=2.0")
    ...
    clean_environment = make_energy_environment(args)
    environment = CertifiedExecutionTrainingEnv(
        clean_environment,
    ...
    callback = CertifiedExecutionMetricsCallback(args, output_dir)
    config = vars(args) | {
        "initialization": "FROM_SCRATCH_NO_DEMONSTRATION",
        "teacher_replay_prefill": False,
        "actor_supervised_warm_start": False,
        "online_certified_execution": True,
        "online_teacher_override": False,
        "sac_action_coordinate": "NORMALIZED_CERTIFIED_SUPPORT_COORDINATE",
        "certified_action_adapter": "physical_action = c + G @ sac_action",
        "replay_action_semantics": "PROPOSED_NORMALIZED_CERTIFIED_SUPPORT_COORDINATE",
        "transition_reward_and_successor_semantics": "ACTUAL_EXECUTED_PHYSICAL_ACTION",
        "algorithm_class": "stable_baselines3.SAC",
        "stable_baselines3_version": sb3_version,
        "torch_version": torch.__version__,
        "gymnasium_version": gymnasium.__version__,
        "numpy_version": np.__version__,
        "environment_class": "CertifiedExecutionTrainingEnv[PersistentEnergyNavigationEnv]",
        "flight_energy_multiplier": args.flight_energy_multiplier,
        "energy_reward_uses_actual_flight_energy": True,
        "certificate": environment.oracle.certificate,
        "observation_fields": list(clean_environment.observation_fields),
        "observation_dimension": int(clean_environment.observation_space.shape[0]),
        "action_space_low": clean_environment.action_space.low.tolist(),
        "action_space_high": clean_environment.action_space.high.tolist(),
        "certified_initial_state_distribution": "RECOVERY_ATLAS_RL_AUTHORITY_DOMAIN",
        "standard_sac_learning_starts": 5000,
        "sac_hyperparameters": {
            "learning_rate": 3e-4,
            "buffer_size": 1_000_000,
            "learning_starts": 5000,
                ...
                for execution_mode in ("POLICY_ONLY", "SYSTEM_WITH_KAPPA"):
                    for evaluation_mode in ("deterministic", "stochastic"):
                        for soc_group in ("full", "low_soc"):
                            ...
                            write_json(
                                output_dir
                                / f"heldout_{execution_mode.lower()}_{evaluation_mode}_{soc_group}_step_{actual_step:07d}.json",
                                evaluation,
                            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standard SB3 SAC with online certified execution")
    ...
    parser.add_argument("--evaluation-seed-base", type=int, default=197_000_000)
    ...
    if not args.smoke and (args.steps != 1_000_000 or args.checkpoint_steps[-1] != 1_000_000):
        parser.error("formal certified execution protocol requires a 1M final checkpoint")
