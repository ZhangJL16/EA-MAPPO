...
SECONDS = 8 * 60 * 60
DEFAULT_OUTPUT = ROOT / 'artifacts/gpu_mpc_parallel_v3_20260926'
...
def seed_from(output: Path, source: Path) -> dict:
    ...
    result = {'protocol': 'gpu_mpc_parallel_seed_v1',
...
def expected_manifest(workers: int) -> dict:
    return {
        'protocol': 'gpu_mpc_parallel_campaign_v1',
...
def run(output: Path, *, workers: int) -> dict:
    if not 1 <= workers <= 3:
        raise ValueError('workers must be 1..3 for the measured 8 GB GPU')
