...
def get_common_args():
    ...
    parser.add_argument('--hrl_ecf_enabled', type=_str2bool, default=True, help='enable embedded high-level energy consequence dynamics head')
    parser.add_argument('--hrl_ecf_consequence_loss_coef', type=float, default=0.05, help='coefficient for supervised ECF energy consequence regression')
    parser.add_argument('--hrl_ecf_feas_loss_coef', type=float, default=0.1, help='coefficient for supervised ECF energy feasibility classification')
    parser.add_argument('--hrl_ecf_policy_loss_coef', type=float, default=0.0, help='coefficient for ECF-guided high-level mode policy regularization')
    parser.add_argument('--hrl_ecf_charge_need_loss_coef', type=float, default=0.0, help='coefficient for explicit charge-needed high-level mode supervision')
    parser.add_argument('--hrl_ecf_charge_need_margin', type=float, default=0.05, help='energy margin below which the ECF auxiliary target prefers charging')
    parser.add_argument('--hrl_ecf_logit_bias_coef', type=float, default=0.1, help='soft charge/order logit bias from ECF feasibility predictions')
    parser.add_argument('--hrl_ecf_utility_target_enabled', type=_str2bool, default=True, help='train ECF feasibility on energy-safe and task-useful high-level outcomes')
    parser.add_argument('--hrl_ecf_utility_charge_margin', type=float, default=0.03, help='energy margin below which charge progress is treated as useful for ECF targets')
    parser.add_argument('--hrl_ecf_utility_reward_threshold', type=float, default=0.0, help='minimum high-level reward for a utility-positive ECF order target')
    parser.add_argument('--hrl_td_energy_critic_enabled', type=_str2bool, default=False, help='enable TD high-level energy critic ensemble for long-horizon energy feasibility')
    parser.add_argument('--hrl_td_energy_critic_ensemble', type=int, default=3, help='number of TD energy critics per high-level agent')
    parser.add_argument('--hrl_td_energy_critic_lr', type=float, default=3e-4, help='learning rate for TD energy critics')
    parser.add_argument('--hrl_td_energy_gamma', type=float, default=0.95, help='discount for TD energy critic targets')
    parser.add_argument('--hrl_td_energy_safe_ratio', type=float, default=0.12, help='battery ratio kept as long-horizon energy safety reserve')
    parser.add_argument('--hrl_td_energy_kappa', type=float, default=1.0, help='uncertainty multiplier for conservative TD energy cost estimate')
    parser.add_argument('--hrl_td_energy_logit_bias_coef', type=float, default=0.0, help='soft negative mode-logit bias from TD energy cost estimate')
    parser.add_argument('--hrl_td_energy_shield_enabled', type=_str2bool, default=False, help='hard-mask clearly unsafe high-level order mode using TD energy critic')
    parser.add_argument('--hrl_td_energy_lagrange_enabled', type=_str2bool, default=False, help='add adaptive Lagrange penalty from TD energy critic to high-level actor loss')
    parser.add_argument('--hrl_td_energy_lagrange_lr', type=float, default=0.01, help='dual update step for TD energy Lagrange multiplier')
    parser.add_argument('--hrl_td_energy_budget', type=float, default=0.025, help='normalized high-level energy cost budget used by the TD energy Lagrange multiplier')
    parser.add_argument('--hrl_td_energy_lagrange_actor_coef', type=float, default=1.0, help='scale for TD energy Lagrange actor penalty')
    parser.add_argument('--hrl_td_energy_obs_energy_index', type=int, default=4, help='index of normalized battery ratio inside the high-level observation')
    parser.add_argument('--hrl_td_energy_target_tau', type=float, default=0.02, help='soft-update coefficient for TD energy target critics')
