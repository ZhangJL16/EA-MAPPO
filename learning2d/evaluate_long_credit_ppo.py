"""Evaluate completed v4 long-credit PPO using the frozen common safety layer."""
...
SOURCE_FILES = (
    ...
    "learning2d/evaluate_long_credit_ppo.py",
)
...
def evaluate(output: Path, *, map_id: int, training_output: Path,
    ...
    if manifest["protocol"] != "dual_constraint_2d_mode_masked_long_credit_v4":
        raise ValueError("not a v4 long-credit PPO training run")
    if manifest["ppo"]["gamma"] != 1.0 or manifest["ppo"]["gae_lambda"] != 1.0:
        raise ValueError("v4 requires gamma=1 and GAE lambda=1")
    ...
    provenance = {
        "protocol": "dual_constraint_2d_long_credit_eval_v4",
        "map_id": map_id,
        "training_gamma": manifest["ppo"]["gamma"],
        "training_gae_lambda": manifest["ppo"]["gae_lambda"],
