"""Isolated long-credit PPO diagnostic for the physical 600-second horizon.

Only GAE lambda changes from the frozen v3 gamma=1/GAE=0.95 condition.
The plant, reward, action mask, model architecture and map split are unchanged.
"""
...
GAMMA = 1.0
GAE_LAMBDA = 1.0
PAIRED_V3_MODEL_SHA256 = "88c77554a96b81f284a52811fda94a0ae63f1e6db685bbf89ea70b898dd4e775"
SOURCE_SET = SOURCE_FILES + (
    "learning2d/finite_horizon_gym.py",
    "learning2d/mode_policy.py",
    "learning2d/train_long_credit_ppo.py",
)
...
def training_manifest(seed: int, total_timesteps: int, n_steps: int,
    ...
    return {
        "protocol": "dual_constraint_2d_mode_masked_long_credit_v4",
        "paired_v3_model_sha256": PAIRED_V3_MODEL_SHA256,
        "seed": seed,
...
def train(output: Path, *, seed: int,
    ...
    manifest = training_manifest(seed, total_timesteps, n_steps,
                                 checkpoint_steps, chosen_device)
    ...
    try:
        if status is None:
            model = PPO(
                ...
                learning_rate=0.0003, gamma=GAMMA, gae_lambda=GAE_LAMBDA,
            ...
            if (model.num_timesteps != status["timesteps"] or
                    model.gamma != GAMMA or model.gae_lambda != GAE_LAMBDA):
                raise RuntimeError("saved model disagrees with status or credit assignment")
