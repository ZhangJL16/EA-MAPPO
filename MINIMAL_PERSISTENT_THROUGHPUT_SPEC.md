...
- 日期：2026-09-20；修订v1.3。状态：**用户已通过导航资格审阅；修复dock-idle后获准B0–B5 diagnostic，不授权最终kill test**。
...
站内idle按已停靠处理，耗能为0，电量保持不变，也不自动补能；补能只能通过`recharge`发生。
站内判定沿用导航到达容差（到站点距离不大于goal_radius），不移动或传送位置。
