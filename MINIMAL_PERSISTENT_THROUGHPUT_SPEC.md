...
- 日期：2026-09-20；修订v1.1。状态：**已授权落实三项修订、独立实现、物理calibration与B0–B5；不授权网络训练或oracle实验**。
...
一个无人机在固定时间内，从持续到达的待办任务中选择执行任务或回站充满电，空队列时强制等待，
在能量约束下最大化完成任务数：
...
- 空队列没有策略决策，强制等待至下一次到达、能量耗尽或T，以先到者为准。
  未来事件时间不提前暴露；不能有任务却主动等待。

站内已满电时屏蔽`recharge`，避免零时间动作循环。其他位置即使满电仍可选择回站。
空队列不允许主动回站或自动充电，这是本版强制等待语义的直接结果。
...
站内强制等待也不自动补能；补能只能通过`recharge`发生。
...
新实现位于独立项目 `/home/zjl/persistent_uav_throughput_v1/`，旧源码与历史结果只读复用。
本轮允许实现、必要的正确性检查、一次小型执行smoke与首个可恢复checkpoint启动检查。
达到启动健康后交还可续跑任务，不监控至完成或自动晋级其他实验。
