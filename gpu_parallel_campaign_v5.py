...
SECONDS = 8 * 60 * 60
DEFAULT_OUTPUT = ROOT / 'artifacts/gpu_mpc_parallel_v5_16workers_20260926'
GIB = 1024 ** 3
...
def seed_from(output: Path, source: Path) -> dict:
    ...
    result = {'protocol': 'gpu_mpc_parallel_seed_v5',
...
def expected_manifest(workers: int) -> dict:
    return {
        'protocol': 'gpu_mpc_parallel_campaign_v5_16workers',
...
def run(output: Path, *, workers: int,
        inherited_timing: tuple[float, float] | None = None) -> dict:
    if not 1 <= workers <= 16:
        raise ValueError('workers must be 1..16 before hardware-specific RAM cap')
    total_ram, available_ram = linux_meminfo_bytes()
    # The 16-process short benchmark peaked at 12.95 GiB USS and left
    # 8.35 GiB MemAvailable on this 23.47 GiB RAM machine, with no swap use.
    # Require idle headroom before starting; never depend on swap for capacity.
    if workers > 8 and total_ram < 22 * GIB:
        raise RuntimeError('16-worker mode requires at least 22 GiB physical RAM')
    minimum_available = (18 if workers > 8 else 12) * GIB
    if workers >= 8 and available_ram < minimum_available:
        raise RuntimeError(f'available RAM below {minimum_available // GIB} GiB')
    ...
    if path.exists():
        ...
        started = inherited_timing[0] if inherited_timing else time()
        deadline_epoch = inherited_timing[1] if inherited_timing else started + SECONDS
        if deadline_epoch <= time():
            raise RuntimeError('inherited campaign deadline has already elapsed')
...
def main() -> None:
    ...
    inherited_timing = None
    if args.seed_from is not None:
        source_manifest = json.loads((args.seed_from / 'manifest.json').read_text())
        inherited_timing = (source_manifest['start_epoch'],
                            source_manifest['deadline_epoch'])
    ...
    with (args.output / 'run.lock').open('w') as lock:
        ...
        print(json.dumps(run(args.output, workers=args.workers,
                             inherited_timing=inherited_timing), sort_keys=True))
