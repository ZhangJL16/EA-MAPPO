...
run_one() {
  ...
  if is_heuristic_method "$method"; then
    command_text="RUN_NAME=$run_name LOG_DIR=$log_dir SEED=$seed EPISODES=$HEURISTIC_EPISODES EPISODE_LIMIT=$EPISODE_LIMIT UAV_N_AGENTS=$UAV_N_AGENTS UAV_TOTAL_ORDERS=$UAV_TOTAL_ORDERS UAV_MAX_ACTIVE_ORDERS=$UAV_MAX_ACTIVE_ORDERS $script"
    if [[ "$DRY_RUN" == "1" ]]; then
      (
